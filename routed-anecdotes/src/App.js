...
import {
  Link, Switch, Route, useRouteMatch, useParams, useHistory
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} id='myForm'>
          ...
          <br/>
          {content.attributes.value} - {author.attributes.value} - {info.attributes.value}        
...
const App = () => {
  ...
  const addNew = (anecdote) => {
    ...
    console.log('param of addNew: ', anecdote)
    setNotification(`a new anecdote ${anecdote.content.substring(0, 14).concat('... ')} created.`)
    setTimeout(() => {
      setNotification('')
    }, 5000*2)
  }
